...
export function ShopProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshCart = async () => { if (!isAuthed) return; try { const cart = await api.get<{data: Cart}>("/cart"); setCartCount(cart.data?.items?.reduce((s, i) => s + i.quantity, 0) || 0); } catch { /* handled */ } };
