...
export const ProductCard = memo(({ product, onQuickView }: { product: Product; onQuickView?: (product: Product) => void }) => {
  ...
  return <article className="group relative overflow-hidden rounded-2xl glass-panel hover:-translate-y-1 hover:shadow-glow">
    <Link to={`/products/${product._id}`} className="block overflow-hidden bg-muted aspect-[4/5]"><img src={api.imgUrl(product.cover || product.images?.[0])} alt={product.name} loading="lazy" decoding="async" className="h-full w-full object-contain p-2 transition-transform duration-500 group-hover:scale-105" /></Link>
    {sale && <span className="absolute left-3 top-3 rounded-full bg-accent px-3 py-1 text-xs font-semibold text-accent-foreground">{isAr ? "خصم" : "Sale"}</span>}
    {out && <span className="absolute inset-x-4 top-1/3 rounded-xl bg-secondary/85 py-2 text-center text-sm font-semibold text-secondary-foreground backdrop-blur">{t.product.out_stock}</span>}
