...
export function ProductCard({ product, onQuickView }: { product: Product; onQuickView?: (product: Product) => void }) {
  ...
  return <article className="group relative overflow-hidden rounded-2xl glass-panel transition-all duration-300 hover:-translate-y-1 hover:shadow-glow">
    <Link to={`/products/${product._id}`} className="block overflow-hidden bg-muted aspect-[4/5]"><img src={api.imgUrl(product.cover || product.images?.[0])} alt={product.name} loading="lazy" className="h-full w-full object-contain p-2 transition-transform duration-500 group-hover:scale-105" /></Link>
