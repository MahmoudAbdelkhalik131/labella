...
export const api = {
  ...
  post: <T>(path: string, body?: unknown) => apiFetch<T>(path, { method: "POST", body: serializeBody(body) }),
  put: <T>(path: string, body?: unknown) => apiFetch<T>(path, { method: "PUT", body: serializeBody(body) }),
  patch: <T>(path: string, body?: unknown) => apiFetch<T>(path, { method: "PATCH", body: serializeBody(body) }),
